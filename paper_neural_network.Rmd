...
and $z_{\backslash l}$ be the complement subset, such that $z_{\backslash l} \cup z_l = x$. The prediction $\hat{F}(x)$ depends on variables in both subsets: $\hat{F}(z_l, z_{\backslash l})$. In general, the functional form of $\hat{F}_{z_{\backslash l}}(z_l)$ will depend on the particular value chosen for $z_{\backslash l}$. If this dependence is not too strong, then the average function 
...
## \texttt{prepare\_data}
Data preparation for plotting is hidden from the end user. The \texttt{prepare\_data} function is responsible for the plotting data creation as explained with the algorithm in chapter 2.3. Besides preparing the plotting data, the function also adds the lower and upper levels for the confidence intervals if wanted by the user. All created visualizations, if no data was stored in the fitted model via the \texttt{options} parameter, use this function to get the necessary data for plotting. Moreover, if the scale parameter of the \texttt{NeuralNetwork} has been set, the data will be descaled back to the original data range for plotting. The functionality for the data preparation is stored in the **plotting_data_preparation.R** file.

If the confidence interval should be added, keep in mind that for each bootstrap iteration the neural network will be refitted with sampled data. This procedure might take some time, since fitting a neural network depends on the available data, the number of hidden layers and the threshold for the minimization of the prediction error. As mentioned, we added the possibility to use parallel computing for decreasing the running time of the bootstrap procedure. Additionally, it is possible to store the plotting data directly in the \texttt{NeuralNetwork} class via the options parameter. This removes the waiting time for the wanted visualizations.

## \texttt{plot\_partial\_dependencies}
After having fit the model, the user can jump directly into producing partial dependence plots with the \texttt{plot\_partial\_dependencies} function. There the user can select all (default behavior), multiple or just a single predictor for which to create the partial dependence plot. Besides that, one can select the lower and upper quantiles for the confidence intervals, the number of bootstrap iterations, whether it should be a \texttt{ggplot} or \texttt{plotly} visualization, if the stored data should be used and if parallel computation should be used. Of course, if the user decides to use the stored data, no computation will take place and hence changing the \texttt{probs}, \texttt{nrepetitions} and \texttt{parallel} parameter will have no effect. The **neural_net_visualization.R ** file contains the plotting function definitions.
...
We added error messages for impossible lower and upper bound combinations, creating plots without specifying the predictors of interest and other inadequate specifications inside the shiny app, for the \texttt{plot\_partial\_dependencies} function and the \texttt{NeuralNetwork} class.

# Example
This chapter will explain how to use the R package. The examples will be based  on the famous iris dataset where a neural network will be used to predict the flower species based on the measurements in centimeters of the variables sepal length and width and petal length and width, respectively, for 50 flowers from each of 3 species of iris.

First of all, let's fit the neural network using the \texttt{NeuralNetwork} class. Compared to the \texttt{neuralnet} package less data preprocessing has to be done by the user. He or she does not have to think about factor columns that should be expanded into dummy variables and scaling does not need to be done manually. By providing the \texttt{options} parameter the data that will be used for visualizing the partial dependencies will be directly computed. The model fitting will take time, since 4000 neural networks will be refitted by the bootstrap procedure and the resulting data will be stored inside the \texttt{NeuralNetwork}.
...
```
Now, it is easily possible to create the partial dependence plots using the \texttt{plot\_partial\_dependencies} function. If the user just provides the model as input, the function will create the partial dependence plots for all predictors without a confidence interval.
...
Let's take a look at the partial dependence plots with bootstrap confidence intervals to capture the uncertainty of the effect. The thick line will represent the average partial dependency and the lower and upper bound the respective quantiles. We can just use the stored data to create the partial dependence plot with the \texttt{probs} and \texttt{nrepetitions} parameter specified as in the \texttt{options} parameter where the model was fit.

```{r}
plot_partial_dependencies(model, use_stored_data = TRUE)
```

Now that the confidence intervals have been added to the partial dependence plots, the interpretation can additionally capture the uncertainty of the effect of a change of the predictor on the response category. Furthermore, if we want to look at one predictor only, we can just specify the \texttt{predictors} parameter. Instead of using the already stored data, we will compute the data for plotting directly.
...
# Conclusion
This paper gave a brief introduction into the concept of interpretability for neural network models. Especially partial dependence plots have been examined and implemented in our \texttt{NeuralNetworkVisualization} package.

Of course, there are some issues using this approach as has been shown in chapter 2.4. Hence, we do not claim that our package is a complete solution for this task. There are a number of steps that could be taken for improvement. 

Firstly, considering the fact that we are only showing one-dimensional relationships, it would be possible to add visualizations for up to three predictors combined. Additionally, Accumulated Local Effect plots could also be added for further interpretability possibilities. Moreover, Individual Conditional Expectation plots are useful for detecting hidden heterogeneity that cannot be seen with partial dependence plots.

However, we created a package that has easy to use functions with which the end user can create visualizations interactively in a shiny app or with the R console. A lot of options are added, from deciding which type of plot should be created to whether or not the computations should be done in parallel. The package functionalities where tested extensively and we tried to minimize the possibilities for inadequate function arguments by adding comprehensible error messages.
